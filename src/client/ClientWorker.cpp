    ...
    _serverSocket(this)
{
    ...
    qRegisterMetaType<eErrorCode>("eErrorCode");
...
void        ClientWorker::onReadSomething()
{
    ...
    std::cout << "Receive tcp packet, size = " << totalSize << std::endl;
    stream.str(std::string(this->_buff, totalSize));
    while ((totalSize - currentSize) >= sizeof(packetHeader))
    {
        if (PacketHelper::readTcpHeader(stream, packetHeader) && (unsigned)(totalSize - currentSize) >= (sizeof(packetHeader) + packetHeader.payloadSize))
        {
            std::cout << "Receive packet with id :" << (int)packetHeader.commandId << std::endl;
...
void        ClientWorker::logoutRequest()
{
    this->_data.getUser().clearFriendList();
    this->endCall();
...
void		ClientWorker::addFriendRequest(QString const &dest)
{
    ...
    for (std::list<Friend>::const_iterator it = user.getFriendList().begin(); it != user.getFriendList().end(); ++it)
    {
        if (it->getName() == dest)
        {
            if (it->getStatus() == FriendRequestPending)
            {
                this->friendValidationRequest(dest, true);
                return;
            }
        }
    }
...
void        ClientWorker::onHangUp()
{
    this->endCall();
}

void        ClientWorker::onSendText(const QString &msg)
{
    if (this->_call != 0)
        this->_call->sendMessage(msg);
}
...
{
    Serializer::deserialize<char>(buff);
...
void        ClientWorker::receiveHostRequest(std::istringstream &buff, int)
{
    ...
    hostReady = hostReady.append(Serializer::serialize<int>(host->getTcpPort()));
    hostReady = hostReady.append(Serializer::serialize<std::string>(username));
...
void        ClientWorker::receiveConnectToPeer(std::istringstream &buff, int)
{
    ...
    std::cout << "Peer tcp server : " << ip.c_str() << ":" << port << std::endl;
    if (this->_call == 0)
    {
        remote = new RemoteCall(*this);
        remote->start(ip.c_str(), port);
        remote->addCallee(username.c_str(), 0);
        this->_call = remote;
    }
}
...
{
	std::string				username = Serializer::deserialize<std::string>(buff);
    std::list<std::string>	list = Serializer::deserialize<std::list<std::string> >(buff);

    if (this->_call != 0)
        this->answerCallRequest(username.c_str(), false);
    else
        emit this->callRequest(username.c_str(), list);
}
...
void        ClientWorker::receiveKO(std::istringstream &buff, int)
{
    ...
    if (errorCode == InvalidUsername || errorCode == InvalidPassword || errorCode == UnavailableUsername
           || errorCode == UserAlreadyConnected || errorCode == AuthentificationTimeOut || errorCode == InvalidCredentials)
        this->_serverSocket.close();
...
void        ClientWorker::receiveWelcome(std::istringstream &buff, int)
{
    ...
    std::cout << "Receive Welcome" << std::endl;
}
...
void        ClientWorker::receiveUdpReady(std::istringstream &buff, int)
{
    ...
    std::cout << "UDPReady received : port = " << port << std::endl;
    emit this->enterInCall();
}
...
void        ClientWorker::receiveSendText(std::istringstream &buff, int)
{
    ...
    std::cout << "Receive a text message from " << id << " : " << msg << std::endl;
    if (this->_call != 0)
    {
        if (this->_call->isHost())
            ((HostedCall*)(this->_call))->onReceiveSendTextMsg(id, msg.c_str());
        QString const &usr = this->_call->getUsernameById(id);
        emit this->sendText(usr, msg.c_str());
    }
}

void        ClientWorker::onClientDisconnectedFromCall()
{
    if (this->_call != 0 && this->_call->isHost() && !this->_call->isStopping())
    {
        QTcpSocket *sock = (QTcpSocket*)(sender());
        HostedCall *call = (HostedCall*)(this->_call);
        if (call->removeCallee(sock))
            this->endCall();
    }
}

void        ClientWorker::onClientKickedFromCall()
{
    if (this->_call != 0 && !this->_call->isHost() && !this->_call->isStopping())
    {
        this->_call->stop();
