...
{
	std::string r = "";
	std::ifstream file(this->_filename.c_str(), std::ios::in | std::ios::binary | std::ios::ate);
...
void					FileSerializer::writeToFile(std::string const& data) const
{
	...
	std::ofstream file(this->_filename.c_str(), std::ios::out | std::ios::binary | std::ios::trunc);
	if (file.is_open() == false)
