...
// Friends
eErrorCode							Database::removeFriend(UserData& asker, std::string const& target)
{
...
eErrorCode							Database::addFriendRequest(UserData& asker, std::string const& target)
{
	UserData* t = this->getUser(target);
	if (t == 0 || *t == asker)
		return UnknownUser;
	...
	if (asker.getListFriend().size() + asker.getListFriendRequest().size() >= MAX_FRIENDS)
		return FriendsMaxedOut;
	t->addFriendRequest(asker);
	return Success;
}

eErrorCode							Database::validateFriendRequest(UserData& asker, std::string const& target)
{
	UserData* t = this->getUser(target);
	if (t == 0)
		return UnknownUser;
...
eErrorCode							Database::refuseFriendRequest(UserData& asker, std::string const& target)
{
	UserData* t = this->getUser(target);
	if (t == 0)
		return UnknownUser;
